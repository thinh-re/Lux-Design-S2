...
class Cargo:
    ...
    def numpy(self) -> np.array:
        return np.array([self.ice, self.metal, self.ore, self.water])
...
class Factories:
    def __init__(self, raw_factories_obs: Dict[str, Dict]) -> None:
        self.player_0_factories: List[
            Factory
        ] = self.__convert_factoriesdict_to_factorieslist(raw_factories_obs["player_0"])
        self.player_1_factories: List[
            Factory
        ] = self.__convert_factoriesdict_to_factorieslist(raw_factories_obs["player_1"])
    ...
    def __convert_factoriesdict_to_factorieslist(
        self, factories_dict: Dict[str, Dict]
    ) -> List[Factory]:
        lst: List[Factory] = []
    ...
    def factories_numpy(
        self, factories: List[Factory], max_factories: int = 2
    ) -> np.ndarray:
        ...
        return np.stack(
            [factory.numpy() for factory in factories[:max_factories]], axis=0
        )
...
class Units:
    def __init__(self, raw_units_obs: Dict[str, Dict]) -> None:
        self.player_0 = self.__convert_unitsdict_to_unitslist(raw_units_obs["player_0"])
        self.player_1 = self.__convert_unitsdict_to_unitslist(raw_units_obs["player_1"])
    ...
    def __convert_unitsdict_to_unitslist(
        self, units_dict: Dict[str, Dict]
    ) -> List[Unit]:
        lst: List[Unit] = []
...
class State:
    def __init__(self, raw_state: Dict[str, Dict]) -> None:
        ...
        self.action_queue_updates_success: int = raw_state[
