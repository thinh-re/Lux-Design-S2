...
class ObservationWrapper(gym.ObservationWrapper):
    ...
    def __init__(self, env: gym.Env) -> None:
        ...
        self.observation_size = (
            ...
            + OurEnvConfig.MAX_UNITS_IN_OBSERVATION
            * Unit.numpy_shape(OurEnvConfig.MAX_ACTIONS_PER_UNIT_IN_OBSERVATION)
            * 2
            + Player.real_env_steps_numpy_shape(self.env_cfg)
        )
        self.observation_space = spaces.Box(-9999, 9999, shape=(self.observation_size,))
    ...
    def convert_obs(obs: Dict[str, Any], env_cfg: EnvConfig) -> Dict[str, npt.NDArray]:
        ...
        our_factory_map = np.array(
            [
                factory.pos
                for factory in observation_obj.player_0.factories.player_0_factories
            ]
        )
        opponent_factory_map = np.array(
            [
                factory.pos
                for factory in observation_obj.player_0.factories.player_1_factories
            ]
        )
        ...
        for agent in obs.keys():
            ...
            units = observation_player.units.numpy(
                agent,
                max_units=OurEnvConfig.MAX_UNITS_IN_OBSERVATION,
